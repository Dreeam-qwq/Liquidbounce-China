            ...
            try {
                ...
                for(final Element element : panel.getElements()) {
                    ...
                    try {
                        ...
                        ClientUtils.getLogger().error("加载设置界面模块元素'" + moduleElement.getModule().getName() + "'时发生错误 (面板名字: " + panel.getName() + ").", e);
                    }
                }
            }catch(final Exception e) {
                ClientUtils.getLogger().error("加载设置界面面板 '" + panel.getName() + "'时发生错误.", e);
            }
        }
    }
