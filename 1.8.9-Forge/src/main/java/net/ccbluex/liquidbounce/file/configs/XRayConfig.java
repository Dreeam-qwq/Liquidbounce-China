...
public class XRayConfig extends FileConfig {
    ...
    protected void loadConfig() throws IOException {
        ...
        if(xRay == null) {
            ClientUtils.getLogger().error("[文件管理器] 无法找到矿透模块.");
            return;
        }
        ...
        for(final JsonElement jsonElement : jsonArray) {
            try {
                ...
                if (xRay.getXrayBlocks().contains(block)) {
                    ClientUtils.getLogger().error("[文件管理器] 跳过透视 '" + block.getRegistryName() + "' 方块，因为此方块已经在列表中.");
                    continue;
                }

                xRay.getXrayBlocks().add(block);
            }catch(final Throwable throwable) {
                ClientUtils.getLogger().error("[文件管理器] 添加矿透方块失败.", throwable);
            }
        }
    }
