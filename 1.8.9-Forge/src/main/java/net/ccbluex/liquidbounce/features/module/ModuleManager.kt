...
class ModuleManager : Listenable {
     ...
     */
    fun registerModules() {
        ClientUtils.getLogger().info("[模块管理器] 加载模块中...")
        ...
        ClientUtils.getLogger().info("[模块管理器] 加载 ${modules.size} 模块.")
    }
    ...
    private fun registerModule(moduleClass: Class<out Module>) {
        try {
            ...
            ClientUtils.getLogger().error("加载: ${moduleClass.name} (${e.javaClass.name}: ${e.message})模块失败")
        }
    }
